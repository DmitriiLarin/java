...
public class NumberFormat {
    ...
    public static String formatNumber(String number) {
        ...
        if (number.startsWith("+") && number.length()>=12 && number.length()<=14) {
            ...
            if (number.length() == 14) {
                countryCode = number.substring(1, 5);
                firstThreeDigits = number.substring(5, 8);
                secondThreeDigits = number.substring(8, 11);
                firstFourDigits = number.substring(11);
            }
